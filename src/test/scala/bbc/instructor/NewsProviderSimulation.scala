...
class NewsProviderSimulation extends Simulation {
    ...
    val textIndexChain =
        ...
        .exec( session => {
            val stories:List[String] = session("stories").as[List[String]]
            val storyUrl:String = stories.apply(random.nextInt(stories.size))
	...
	val scn = scenario("NewsProviderSimulation")
        ...
        .exec( session => {
            val categories:List[String] = session("categories").as[List[String]]
            val randomCategory:String = categories.apply(random.nextInt(categories.size))
